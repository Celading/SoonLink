...
SoonLink Core 现在同时维护 GitHub 与 GitCode 两套公开自动化入口：

- GitHub 负责完整的多平台 Release、Docker 和 Homebrew 发布链。
- GitCode 负责贴近公开仓的 Linux 主线 CI 与 tag Linux bundle 组装，目录固定在 `.gitcode/workflows/`。

这样可以把“公开源仓校验”和“完整跨平台制品发布”分层处理，避免把所有发布责任都压在同一平台上。
...
- `.gitcode/workflows/core-ci.yml`
  用于 GitCode Euler runner 上的公开仓主线校验。
  - runner 使用 GitCode 默认的 `euleros-2.10.1`。
  - 通过 `checkout-action@0.0.1` 拉取代码后，在 `repo_workspace` 中执行。
  - 会先做公开边界审计，再安装 Linux 构建依赖、下载仓颉 SDK / `stdx`、克隆 `Ignite / lisi / jinguiSSL`，最后在临时投影工作副本中执行 `cjpm build` 与 `cjpm test`。
