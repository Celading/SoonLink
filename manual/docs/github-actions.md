...
- `.github/workflows/core-ci.yml`
  ...
  - 若同时配置 `CANGJIE_STDX_RELEASE_VERSION`，会优先下载官方预打包 `stdx` release；未配置时，继续回退到当前的 `stdx` 源码构建流程。
...
- `.github/workflows/release-artifacts.yml`
  ...
  - 当 OpenHarmony 工具链可解析时，会追加：
    - `ohos-aarch64`
  - 若已配置 `CANGJIE_STDX_RELEASE_VERSION`，release 打包也会直接复用对应平台的官方 `stdx` 预构建包，减少每个 job 自行编译 `stdx` 的波动。
...
- `CANGJIE_STDX_RELEASE_VERSION`
  可选。若配置，会优先下载 `https://gitcode.com/Cangjie/cangjie_stdx/releases/download/v<version>/cangjie-stdx-<platform>-<version>.zip` 形式的官方预打包 `stdx`，而不是在 CI 中现编。未配置时，继续走源码构建。
...
- `scripts/install_cangjie_ci.sh`
  ...
  - 已支持优先使用官方 `stdx` release 包；若缺少 release version 或 release 包不可用，再回退到源码构建。
- `scripts/build_release_target.sh`
  统一执行“按目标构建 + 打包”。
