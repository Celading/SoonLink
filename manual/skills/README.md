...
这里放的是可复用、可共享的 SoonLink 使用范式，而不是内部团队规约。

## 推荐先看

- [CLI 与 MCP 技能手册](./cli-and-mcp.md)
  - 适合第一次接入 SoonLink 节点，或想把 SoonLink 接进 agent 自动化流程时使用。

## 最值得先掌握的技能

- **版本与能力确认**
  - 先用 `soonlnk version` 和 `soonlnk capabilities` 确认当前版本号、能力标签和运行状态。
- **状态先行**
  - 先看 `config` / `status` / `connectors`，再做设备登记、授信和传输。
- **设备登记与授信**
  - 手工登记节点、PIN 配对、显式 trust 是最稳的联调入口。
- **路径总结后再传输**
